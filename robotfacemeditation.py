...
def main():
    ...
    for excel_row_idx, idx_val in enumerate(indices, start=2):  # row 1 = header in all sheets
        ...
        # Count contiguous runs of "smoothfaced" per category
        ...
        for i in range(n):
            ...
            # If category changed, break any ongoing run
            if cat != prev_cat:
                for k in in_look:
                    in_look[k] = False
                prev_cat = cat
        ...
        # Write counts to sheets 2..5 (column C)
        ...
        print(f"✅ {idx5} [{folder}] crisis via {tag_src or 'unknown'} ({crisis_col}) | "
              f"looks → pre_no={counts['pre_no']}  pre_med={counts['pre_med']}  "
    ...
    try:
        wb.save(OUTPUT_XLSX)
        print(f"\n💾 Wrote robot-face look counts to '{OUTPUT_XLSX}' (Sheets 2–5, column C).")
    except Exception as e:
    ...
    # Problem buckets (optional, helpful lists)
    if not_found_csv:
