...
def main():
    ...
    if not os.path.isdir(speed_dir):
        print("Error: 'speed' folder not found.")
        return

    files = [f for f in os.listdir(speed_dir) if f.endswith(".txt")]
    if not files:
        print("No .txt files in 'speed'.")
        return
    ...
    for name in files:
        ...
        p, r = load_speed_data(os.path.join(speed_dir, name))
        if len(p) == 0:
