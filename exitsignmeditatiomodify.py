...
COL_COUNT_1BASED  = 5              # write signage look counts to column E

LOOK_STARTS_TOKEN = "signage"      # contiguous runs whose LookingAt startswith "signage"
...
def main():
    ...
    for excel_row_idx, (idx5, crisis_time) in enumerate(zip(indices, crisis_times), start=2):  # row 1 is header
        ...
        # Meditation state (roomEvent preferred else Event; if both missing → all False)
        in_med, used_med_col = build_in_meditation(df, colmap["roomevent"], colmap["event"], len(df))

        # Count contiguous “Signage...” looks in four buckets
        ...
        for i in range(len(df)):
            ...
            val = look_vals[i]
            # Count contiguous runs whose LookingAt starts with "signage"
            if isinstance(val, str) and val.startswith(LOOK_STARTS_TOKEN):
                if not in_run[cat]:
                    counts[cat] += 1
                    in_run[cat] = True
            else:
        ...
        # Write signage look counts to sheets 2..5 (column E)
        ...
        for sheet_1based, val in zip(SHEET_IDX_FOR_CAT, values):
            ...
            ws.cell(row=excel_row_idx, column=COL_COUNT_1BASED).value = int(val)

        ok += 1
        print(f"✅ {idx5} [{folder}] t_crisis={float(crisis_time):.3f}  "
    ...
    try:
        wb.save(OUTPUT_XLSX)
        print(f"\n💾 Wrote signage look counts to column E on Sheets 2–5 in '{OUTPUT_XLSX}'.")
    except Exception as e:
