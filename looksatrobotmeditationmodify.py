...
# UPDATED: search only noshookmodified and its baseline
SEARCH_FOLDERS = [
    "noshookmodified",
    os.path.join("noshookmodified", "baseline"),
]

# Sheets 2..5 (1-based) -> indexes [1],[2],[3],[4] in openpyxl
...
def normalize_text(s: str) -> str:
    """Trim, unicode-normalize, collapse spaces; keep original case to avoid surprises in headers."""
    if s is None:
        return ""
...
def norm_header_token(s: str) -> str:
    """Lower/strip and remove spaces/._- for robust header matching."""
    return re.sub(r"[ _.\-]+", "", normalize_text(s).lower())

def find_columns(header, want_keys):
    """Relaxed (case-insensitive) FIRST-match by normalized token."""
...
def crisis_time_from_tag_column(df: pd.DataFrame, time_col: str, tag_col_name: str):
    """
    In the given tag column (case-insensitive, substring):
      1) first 'estimated shook' -> t[row]
      2) else first 'shook'      -> t[row]
      3) else first '0.2 seconds'-> t[row] + 0.229
      else -> None
    """
...
def build_meditation_state(df: pd.DataFrame, roomevent_idx: int, event_idx: int, n_rows: int):
    ...
    Tags: 'entered meditation area' / 'exited meditation area' (case-insensitive substrings).
    Returns (in_med: np.ndarray or None, used_col_name or None)
    """
...
def count_robot_looks_by_category(df: pd.DataFrame, time_col: str,
                                  ...
                                  look_col_idx: int):
    """
    Count contiguous 'lookingAt == robot' episodes, split into 4 categories by
...
def main():
    ...
    for row_idx_excel, idx_val in enumerate(indices, start=2):  # header row is 1
        ...
        # ---- Crisis detection:
        # Use the RIGHTMOST 'robotEvent' if there are multiple; else use 'Event'.
        rightmost_robot_evt_idx = find_rightmost_col(list(df.columns), "robotEvent")
        ...
        if crisis_time is None:
            record_skip("no_crisis_tag",
                        f"{idx5}: SKIP (no 'estimated shook'/'shook'/'0.2 seconds' in {crisis_col_name}) [{folder}]")
            continue
